...
class TestScorecardHtml:
    ...
    def _make_card(self) -> "ScoreCard":
        ...
        return ScoreCard(
            ...
            overall_grade="B",
            categories=[
                CategoryScore(name="Security", score=74, grade="B", finding_count=2),
                CategoryScore(name="Encryption", score=90, grade="A", finding_count=0),
                CategoryScore(name="IAM Hygiene", score=80, grade="A", finding_count=1),
                CategoryScore(name="Cost Efficiency", score=100, grade="A", finding_count=0),
                CategoryScore(name="Tagging", score=30, grade="F", finding_count=3),
            ],
            top_issues=[
                Finding(
                    rule_id="SEC-001",
                    severity=Severity.critical,
                    title="S3 publicly accessible",
                    description="desc",
                    remediation="fix",
                ),
                Finding(
    ...
    def test_contains_infracanvas_dev_founding(self):
        """Score card must link to infracanvas.dev/founding for upgrade CTA."""
        with tempfile.TemporaryDirectory() as tmpdir:
            out = Path(tmpdir) / "score.html"
            export_scorecard(self._make_card(), out)
            content = out.read_text()
            assert "infracanvas.dev/founding" in content

    def test_contains_generated_by_attribution(self):
        """Score card must have attribution footer."""
        with tempfile.TemporaryDirectory() as tmpdir:
            out = Path(tmpdir) / "score.html"
            export_scorecard(self._make_card(), out)
            content = out.read_text()
            assert "Generated by InfraCanvas" in content

    def test_contains_grade_a_color(self):
        """Grade A color #22c55e must appear in CSS."""
        with tempfile.TemporaryDirectory() as tmpdir:
            out = Path(tmpdir) / "score.html"
            export_scorecard(self._make_card(), out)
            content = out.read_text()
    ...
    def test_contains_encryption_dimension_color(self):
        """Encryption dimension color #a855f7 must appear in HTML."""
        with tempfile.TemporaryDirectory() as tmpdir:
            out = Path(tmpdir) / "score.html"
            export_scorecard(self._make_card(), out)
            content = out.read_text()
            assert "#a855f7" in content

    def test_contains_og_title(self):
        """OG meta tag og:title must be present."""
        with tempfile.TemporaryDirectory() as tmpdir:
            out = Path(tmpdir) / "score.html"
            export_scorecard(self._make_card(), out)
            content = out.read_text()
    ...
    def test_contains_unlock_finding_details(self):
        """Upgrade CTA copy must say 'Unlock finding details'."""
        with tempfile.TemporaryDirectory() as tmpdir:
            out = Path(tmpdir) / "score.html"
            export_scorecard(self._make_card(), out)
            content = out.read_text()
            assert "Unlock finding details" in content

    def test_no_svg_circle_gauge(self):
        """Old SVG circle gauge design must not appear."""
        with tempfile.TemporaryDirectory() as tmpdir:
            out = Path(tmpdir) / "score.html"
            export_scorecard(self._make_card(), out)
            content = out.read_text()
            assert "stroke-dasharray" not in content
            assert "score-circle" not in content

    def test_contains_all_5_dimensions(self):
        """All 5 dimension names must appear in the HTML."""
        with tempfile.TemporaryDirectory() as tmpdir:
            out = Path(tmpdir) / "score.html"
            export_scorecard(self._make_card(), out)
            content = out.read_text()
            for dim in ["Security", "Encryption", "IAM Hygiene", "Cost Efficiency", "Tagging"]:
                assert dim in content, f"Missing dimension: {dim}"

    def test_contains_grade_letter_grade_b_color(self):
        """Grade B color #4ade80 must appear when overall_grade is B."""
        with tempfile.TemporaryDirectory() as tmpdir:
            out = Path(tmpdir) / "score.html"
            export_scorecard(self._make_card(), out)
            content = out.read_text()
